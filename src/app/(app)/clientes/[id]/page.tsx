...
import { ArrowLeft, Phone, MapPin, Cake, History, Clock } from "lucide-react";
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div className="lg:col-span-2 space-y-5">
            <div className="grid grid-cols-2 gap-4">
              <StatTile label="Visitas totales" value={resumen.totalVisitas} icon={<History size={17} />} tono="terracota" />
              <StatTile
                label="Última visita"
            ...
            <Card>
              <CardHeader title="Historial" subtitle="Hospedajes de este cliente" />
              {hospedajes.length === 0 ? (
                <p className="text-sm text-[var(--color-gris-medio)] py-6 text-center">Todavía no registra actividad de compra.</p>
              ) : (
                <ul className="space-y-1">
                  {[
                    ...hospedajes.map((h) => ({ key: h.id, fecha: h.checkIn, texto: `Hospedaje · Hab. ${h.habitacion}` })),
                  ]
                    ...
                    .map((item) => (
                      <li key={item.key} className="flex items-center justify-between py-2.5 border-b border-[var(--color-gris-claro)]/20 last:border-0 text-sm">
                        ...
                        <span className="text-xs text-[var(--color-gris-medio)]">{new Date(item.fecha).toLocaleDateString("es-PE")}</span>
                      </li>
                    ))}
                </ul>
              )}
