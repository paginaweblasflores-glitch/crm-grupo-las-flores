...
import { Download, BedDouble, CalendarCheck } from "lucide-react";
...
export default function HospedajePage() {
  ...
  function exportar() {
    exportarCSV(
      "hospedaje-umaru",
      ["Cliente", "Check-in", "Check-out", "Habitación", "Canal"],
      filtrados.map((h) => [h.clienteNombre, h.checkIn, h.checkOut, h.habitacion, h.canal])
    );
  }
    ...
    <>
      ...
      <main className="flex-1 p-8 animate-fade-in space-y-5">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <StatTile label="Huéspedes distintos" value={new Set(HOSPEDAJES.map((h) => h.clienteId)).size} icon={<CalendarCheck size={18} />} tono="verde" />
        </div>
        ...
        <Card padding="p-0 pt-5">
          ...
          <Table>
            <Thead>
              <Th>Huésped</Th><Th>Check-in</Th><Th>Check-out</Th><Th>Habitación</Th><Th>Canal</Th>
            </Thead>
            <tbody>
              {filtrados.map((h) => (
                <Tr key={h.id}>
